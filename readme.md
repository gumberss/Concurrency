...
Vamos imaginar que estamos implementando uma fila no banco de dados para um sistema que tem cinco instâncias do sistema que está lendo as mensagens dessa fila. Nesse caso, uma mensagem pode levar um tempo indefinido para terminar de processar e como são várias instâncias tentando processar ao mesmo tempo, muito provavel que vamos ter um alto nível de concorrência por registro (mensagem), pois todas as instâncias vão querer processar aquele registro na tabela. Nesse caso talvez uma boa opção seja utilizar um nível de isolamento maior evitando que a mensagem seja processada mais de uma vez e encontrar uma forma de avisar o banco de dados para pular registros travados por outra transação. Mais uam vez, com Postgres você consegue alcançar isso com o select for update, utilizando o [skip locked](https://www.postgresql.org/docs/current/sql-select.html#:~:text=With%20SKIP%20LOCKED%20%2C%20any%20selected,accessing%20a%20queue%2Dlike%20table.). Um exemplo bem didático da implementação desse exemplo está aqui [neste vídeo](https://youtu.be/jTLP5DrIocA?si=qTbcturjGDg3X25U). Nesse caso estamos utilizando o nível mais alto de isolamento pois temos um certo alto grau de confiança para dizer que o nível de concorrência é muito alto e vai acontecer com muita frenquência tentativas de alterações no mesmo registro ao mesmo tempo. Por esse motivo utilizamos uma trava pessimista, pois estamos esperando o pior caso, ou seja, alto nível de concorrência.

Por outro lado, em um cenário real de conta de um banco, muito provavelmente vamos ter muitas atualizações ocorrendo ao mesmo tempo mas em registros diferentes, dificilmente vamos ter a mesma pessoa fazendo duas compras ao mesmo tempo. Note que eu falei difícilmente e não que isso seria impossível, uma vês que a pessoa pode estar comprando em uma loja e ao mesmo tempo um familiar comprando algum produto no mercado online, ou até mesmo uma conta sendo paga automaticamente pelo sistema na mesma hora. Problemas de concorrências ainda sim vão acontecer, mas tendem a ser em uma quantidade menor. Nesses casos podemos assumir a responsabilidade de controlar casos de exceção, reduzindo a responsabilidade do banco de dados, mas ainda sim tirando vantagem o máximo possível. Nesse caso, informamos ao banco de dados que ele só pode atualizar determinada informação se o valor dela não mudou, se por algum motivo o dado foi alterado, então pedimos ao banco de dados para nos avisar e caso ele nos avise, abortamos nossa transação e começamos o processo desde o início novamente até conseguirmos atualizar o registro. Nesse caso podemos utilizar o nível de isolamento mais baixo, como por exemplo o [Read Committed](https://learn.microsoft.com/en-us/sql/odbc/reference/develop-app/optimistic-concurrency?view=sql-server-ver16), ou seja, conseguimos reduzir a quantidade de travas que o banco de dados vai ter que fazer, consequentemente reduzindo a responsabilidade de controloar a concorrência e a carga imposta sobre ele.

![image](https://github.com/gumberss/Rinha-Sharding/assets/38296002/b0dd5941-c482-4475-ac46-0bd26b414e77)

No exemplo acima, na mesma hora que a Alice foi fazer uma compra, o sistema pagou uma conta automaticamente. As transações acabaram rodando em concorrência e mesmo que a transação da Alice tenha iniciado antes, ela acabou terminando depois e quando o registro foi ser atualizado para remover o saldo, a segunda aprte da verificação do update acabou "falhando", pois o saldo da alice que no início da transação era de 200 reais, agora era de 0 reais, pois a transação do sistema persistiu a informação antes e como o isolamento da transação da Alice é Read Commited, ela acaba vendo a informação que foi comitada quando foi dar o update. Nós que assumimos a responsabilidade de controlar a concorrência, verificamos que nenhum registro foi alterado, abortamos a transação (tirando proveito da atomicidade (A)cid) e reiniciamos o processo desde o início. Muitos bancos de dados provêm uma funcionalidade própria para essas situações chamado de compare and substitute ou [compare and swap](https://docs.datomic.com/pro/transactions/transaction-functions.html). O importante denotar nesse caso é que estamos sendo otimistas e confiando que vamos ter poucos conflitos, caso contrário teriamos que ficar abortando e reiniciando a nossa transação varias vezes, disperdiçando recursos valiosos do nosso sistema e banco de dados. Se escolhermos usar o lock otimista quando esperamos ter muita concorrência, há a possibilidade do sistema perder performance em relação a utilizar lock pessimista.

Ainda pensando em lock otimista, poderiamos tentar baixar um pouco mais o nível de isolamente para Read Uncommitted por exemplo, mas do jeito que implementamos podemos acabar gerando inconsistências no banco de dados, conforme o exemplo abaixo:
